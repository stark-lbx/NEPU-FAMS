...
logger = get_logger("flow_consumer")


class FlowMqConsumer:
    ...
    def _handle_message(self, data: dict):
        ...
        if cache_key:
            ...
            logger.info(f"流程服务删除缓存: {cache_key}")

        if cache_pattern:
            keys = self.redis.keys(cache_pattern)
            if keys:
                self.redis.delete(*keys)
                logger.info(f"流程服务批量删除缓存: {cache_pattern}, 数量: {len(keys)}")

    def start(self):
        self.consumer.start()
        logger.info("流程服务MQ消费者启动成功")